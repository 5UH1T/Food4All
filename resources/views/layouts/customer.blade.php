        ...
        <aside :class="sidebarOpen ? 'w-64' : 'w-20'"
            ...
            <nav class="side-navbar flex-1 px-3 py-5 space-y-2 overflow-y-auto">
                ...
                <a href="{{ route('customer.profile') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition {{ request()->routeIs('customer.settings') ? 'active' : '' }}">
                    <i class="fa-solid fa-gear"></i>
                    <span x-show="sidebarOpen">Profile</span>
                </a>

                <a href="{{ route('customer.orders') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition {{ request()->routeIs('customer.orders') ? 'active' : '' }}">
                    <i class="fa-solid fa-box-archive"></i>
                    <span x-show="sidebarOpen">Orders</span>
                </a>

                <a href="{{ route('customer.payments') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition {{ request()->routeIs('customer.payments') ? 'active' : '' }}">
                ...
                <div class="relative" x-data="{ open: false }">
                    ...
                    <button @mouseenter="open = true"
                        ...
                        <div class="text-left hidden md:block">
                            <p class="text-sm font-semibold text-gray-700">customer User</p>
                            <p class="text-xs text-gray-500">customeristrator</p>
                        </div>
                    </button>
